#!/usr/bin/env python3
#
# Complete Final LogAI Application - All Issues Fixed + Advanced Features
# Fixed: Admin credentials display, modal size, default log viewer selection
# Added: Admin password reset, integrated syntax highlighting
#
import os
import dash
...
from gui.callbacks import pattern, telemetry, anomaly_detection, clustering, utils, ai_analysis, log_viewer
                ...
                dbc.Card([
                    ...
                    dbc.CardBody([
                        ...
                        dbc.Nav([
                            ...
                            dbc.NavItem(dbc.NavLink([html.I(className="fas fa-project-diagram me-1"), "Clustering"], 
                                                  href="/workspace/clustering", id="nav-clustering", active=False, className="small")),
                            dbc.NavItem(dbc.NavLink([html.I(className="fas fa-exclamation-triangle me-1"), "Anomaly"], 
                                                  href="/workspace/anomaly", id="nav-anomaly", active=False, className="small")),
...
def update_workspace_content(pathname, project_data):
    ...
    if page == "viewer":
        ...
        return clustering_page.layout
    elif page == "anomaly":
        return anomaly_page.layout
...
"""
# Persistent project files callback
@callback(
    Output("persistent-project-files-list", "children"),
    [Input("current-project-store", "data"),
     Input("workspace-upload", "contents"),
     Input("refresh-files-icon", "n_clicks")],
    [State("workspace-upload", "filename")],
    prevent_initial_call=True
)
def update_persistent_project_files(project_data, uploaded_contents, refresh_clicks, filenames):
    if not project_data or not project_data.get("project_id"):
        return html.P([html.I(className="fas fa-info-circle me-2"), "No project selected"], className="text-muted")
    triggered = ctx.triggered_id

    project_id = project_data["project_id"]
    if triggered == "workspace-upload" and (uploaded_contents and filenames):
        for content, filename in zip(uploaded_contents, filenames):
            dbm.save_uploaded_file(project_id, content, filename)
    ...
    for filename, _, original_name, file_size, _ in files:
        ...
        text_extensions = ['.txt', '.log', '.csv', '.json', '.xml', '.yaml', '.yml', '.md', '.py', '.conf']
        is_viewable = any(original_name.lower().endswith(ext) for ext in text_extensions)
                    ...
                    dbc.Col([
                        dbc.ButtonGroup([
                            dbc.Button([html.I(className="fas fa-eye")], 
                                     id={"type": "load-file", "index": filename},
                                     color="outline-info", size="sm", 
...
@callback(
    [Output("nav-viewer", "active"),
     ...
     Output("nav-clustering", "active"),
     Output("nav-anomaly", "active"),
